<div>
    ...
    <x-form-section submit="createApiToken">
        ...
        <x-slot name="form">
            ...
            @if (Laravel\Jetstream\Jetstream::hasPermissions())
                <div class="col-span-6 sm:col-span-4">
                    <flux:label for="permissions" class="mb-1">{{ __('Permissions') }}</flux:label>
                    <flux:pillbox
                        id="permissions"
                        name="permissions"
                        wire:model="createApiTokenForm.permissions"
                        multiple
                        placeholder="{{ __('Choose permissions...') }}"
                        class="w-full mt-2"
                    >
                        @foreach (Laravel\Jetstream\Jetstream::$permissions as $permission)
                            <flux:pillbox.option value="{{ $permission }}">{{ $permission }}</flux:pillbox.option>
                        @endforeach
                    </flux:pillbox>
                </div>
            @endif
    ...
    <!-- Token Value Modal -->
    <x-dialog-modal wire:model.live="displayingToken" class="w-full max-w-xl">
        <x-slot name="title">
            {{ __('API Token') }}
        </x-slot>
    ...
    <x-dialog-modal wire:model.live="managingApiTokenPermissions" class="w-full max-w-xl">
        <x-slot name="title">
